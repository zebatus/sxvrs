...
class ObjectDetector_local(ObjectDetectorBase):
    ...
    def __init__(self, cnfg):
        ...
        logging.info(f"ObjectDetector: Num GPUs Available: {self.count_GPU}")
        #self.filename_model = os.path.dirname(os.path.dirname(os.path.realpath(__file__))) + f'/models/{model}/frozen_inference_graph.pb'
        self.filename_model = self.cnfg.object_detector_local_model_filename os.path.dirname(os.path.dirname(os.path.realpath(__file__))) + f'/models/{model}/frozen_inference_graph.pb'
        if not os.path.isfile(self.cnfg.object_detector_local_model_filename):            
            logging.error(f'There is no model for TF: {self.filename_model}') 
        else:
    ...
    def load_image(self, filename):    
        if os.path.isfile(filename):
            logging.debug(f"ObjectDetector: open file '{filename}' ({self.frame_width}, {self.frame_height})")
            try:
                self.image = cv2.imread(filename)
                self.original_height, self.original_width, self.original_channels = self.image.shape
                return True
            except Exception as ex:
                logging.exception(f"Error in ObjectDetector: can't open image '{filename}' ({self.frame_width}, {self.frame_height}) \n {cmd}")
                raise ex
        else:
            logging.error(f"Can't find file: '{filename}'")
        ...
        if self.load_image(filename):
            ...
            for i in range(len(boxes)):
                ...
                if scores[i] > self.threshold:
                    logging.debug(f'Object detected! class:{classes[i]} score:{scores[i]}')
            ...
            logging.debug(f"ObjectDetector Elapsed Time:{result['elapsed']} : \n {result}", )
